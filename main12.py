...
def SetQuestion(nameList, num):
    ...
    pb_num = 0 #현재, 몇번 맞는 동작을 수행했는지 (즉, 모션 m1+m2+m3 의 값임)
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        for j in range(3):
            ...
            gameDisplay.blit(prob_promptImg, (promptX, promptY))
            textline = font.render('제한 시간 : {:.1f}'.format(limit_time), True, (0, 255, 0))
            gameDisplay.blit(textline, (namingBox_x + 100, 100))
...
def add_input_to_quiz(file_name, problem_index, motion_function_names):  # json 파일, 문제번호(랜덤), 모션함수 이름 리스트
    ...
    with open(file_name, 'r') as file:
        ...
        for i, motion_function_name in enumerate(motion_function_names):
            ...
            #answer_name_index[i] = motion_function_name #문제에 포함되는 모션이름을 저장해 놓음
...
def answer_info(file_name, problem_index, motion_function_names):  # 정답 체크하는 화면
    ...
    with open(file_name, 'r') as file:
        ...
        procedure = int(problem['start'])  # 행동 순서 0, 1, 1, 2, 3 : 0번 행동 -> 1번 행동 -> 1번 행동 ...
        limit_time = int(problem['limit_time'])  # 제한시간
...
def check_motion(count, cur_motion, answer_motion):
    ...
    if len(cur_motion) > 0 and len(answer_motion) > 0:
        ...
        # cur_motion와 answer_motion의 동작을 비교합니다.
        similarity = compare_motion(cur_motion, answer_motion)
       # print(similarity)
        # 유사도가 임계값 미만이면 동일한 것으로 간주합니다
        temporal = 0.9964
