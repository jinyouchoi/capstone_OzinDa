...
retry = load_image("C:\\capston\\객체\\retry.png", (370, 100))
...
warning_barImg = load_image("C:\\capston\\객체\\warning_bar.png", (480, 80))
# 투명화
...
warning_textImg = load_image("C:\\capston\\객체\\warning_text.png", (480, 80))
...
def level_select(nameList):
    ...
    move_size = 10
    # 레벨 단계별로 나오는 거
    level_1_Button = Button2(level1, 65, 550, level1.get_width(), level1.get_height(), level1, 65+move_size, 550+move_size, nameList, 0, SetQuestion)
    level_list.append(level_1_Button)
    level_2_Button = Button2(level2, 330, 460, level2.get_width(), level2.get_height(), level2, 330+move_size, 460+move_size, nameList, 1, SetQuestion)
    level_list.append(level_2_Button)
    level_3_Button = Button2(level3, 665, 530, level3.get_width(), level3.get_height(), level3, 665+move_size, 530+move_size, nameList, 2, SetQuestion)
    level_list.append(level_3_Button)
    level_4_Button = Button2(level4, 840, 300, level4.get_width(), level4.get_height(), level4, 840+move_size, 300+move_size, nameList, 3, SetQuestion)
    level_list.append(level_4_Button)
    level_5_Button = Button2(level5, 1100, 450, level5.get_width(), level5.get_height(), level5, 1100+move_size, 450+move_size, nameList, 4, SetQuestion)
    level_list.append(level_5_Button)
    level_6_Button = Button2(level6, 1380, 370, level6.get_width(), level6.get_height(), level6, 1380+move_size, 370+move_size, nameList, 5, SetQuestion)
...
def SetQuestion(nameList, num):
    ...
    ### 문제마다 반복할 부분!!
        ...
        #for level in range(num, how_many_level):
            ...
            warning = ChangeImg(no_warning_barImg, warning_barImg, 180, 80, warning_barImg.get_width(), warning_barImg.get_height)
            ...
            with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
                ...
                if not_correct:
                    ...
                    gameDisplay.blit(retry, (610, 700))
                    retryButton = Button(retry, 610, 700, retry.get_width() + 30, retry.get_height() + 30, retry, 610, 700, None)
