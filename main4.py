    ...
    if not iscap:
        ...
        for i in range(3):
            ...
            for j in range(5):
                ...
                capture_complete2 = font.render("for 5 seconds.", True, (255, 255, 255))
                gameDisplay.blit(capture_complete2, (800, 300))

                # 카운트 다운 출력
                count_down = font_countdown.render(str(5 - j), True, (0, 0, 0))
                gameDisplay.blit(count_down, ((380, 320)))
            ...
            # 캡처 시작 문구 띄우기
            capture_complete5 = font.render("Please take a pose with your", True, (255, 255, 255))
            gameDisplay.blit(capture_complete5, (800, 200))

            capture_complete6 = font.render("entire body.", True, (255, 255, 255))
            gameDisplay.blit(capture_complete6, (800, 300))
            pygame.display.flip()
...
def SetQuestion(nameList, num):
    ...
    with mp_pose.Pose(min_detection_confidence=0.5, min_tracking_confidence=0.5) as pose:
        ...
        for j in range(3):
            ...
            count_down = font_countdown.render(str(3 - j), True, (0, 0, 0))
...
def check_answer(file_name, problem_index, motion_function_names): #정답 체크하는 화면
    ...
    with open(file_name, 'r') as file:
        ...
        procedure = problem['procedure'] #행동 순서 0, 1, 1, 2, 3 : 0번 행동 -> 1번 행동 -> 1번 행동 ...
        limit_time = problem['limit_time'] #제한시간

    with open('frame_'+motion_function_names+'.csv') as file2:
        df = pd.read_csv(file2)
        answer_motion = df.iloc[:, 33:].values
